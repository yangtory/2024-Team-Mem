document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll(".table_div table tr.sales_list")
    .forEach((row) => {
      row.addEventListener("click", () => {
        ...
        if (sdate && id) {
          ...
          document.querySelector('input[name="r_sdate"]').value =
            sdate;
          document.querySelector('input[name="r_uid"]').value = id;
          document.querySelector('input[name="i_title"]').value =
            title;
          document.querySelector('input[name="i_price"]').value =
            price;
        } else {
          // 선택된 행이 없는 경우
