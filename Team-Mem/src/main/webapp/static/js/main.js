document.addEventListener('DOMContentLoaded', async () => {
    ...
    join_btn?.addEventListener('click', async () => {
        ...
        const comp_tel = join_form.querySelector("input[name='c_tel']");
        const comp_addr = join_form.querySelector("input[name='c_addr']");
        ...
        if ((cname.value && !comp_addr.value) || !comp_tel.value) {
            error_box_join.innerHTML = '업체 주소와 전화번호를 입력해주세요';
            return false;
        }
        alert('회원가입이 완료되었습니다.');
        join_form.submit();
