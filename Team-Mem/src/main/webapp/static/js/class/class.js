  ...
  const renderCalendar = async () => {
    ...
    for (let j = 0; j < day_all.length; j++) {
      ...
      const schedules = json.filter(
        (schedule) =>
          schedule.c_sdate <= formattedDate &&
          schedule.c_edate >= formattedDate
      );
        ...
        if (
          ...
          schedules.forEach(async (schedule) => {
            ...
            if (!right.innerHTML.includes(scheduleText)) {
              ...
              rightSpanTeacher.textContent =
                "강사 " + scheduleTeacher.t_name;
