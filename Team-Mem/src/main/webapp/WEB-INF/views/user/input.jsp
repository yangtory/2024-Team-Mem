        ...
        <input type="hidden" class="us_ccode" placeholder="업체코드" name="us_ccode" value="${CCODE}" readonly/>
            ...
            <c:if test="${UMINFO.size() > 1}">
                <c:forEach items="${UMINFO }" var="UMINFO">
                    <div class="selectBox">
                        <select name="r_iseq" class="selectBtn">
                    ...
                    <label>수강권횟수</label><input class="r_icount" placeholder="수강권횟수" name="r_icount" value="${UMINFO.r_icount }" readonly >
                    <label>시작일</label><input class="r_sdate" type="date" name="r_sdate" value="${UMINFO.r_sdate }" >
                    <label>종료일</label><input class="r_edate" type="date" name="r_edate" value="${UMINFO.r_edate }" >
                </c:forEach>
            </c:if>
            <c:if test="${UMINFO.size() <= 1 }">
